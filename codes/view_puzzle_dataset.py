...
# Set the name of the data folder you want to inspect
DATA_FOLDER_NAME = "prepared_data_final_move_hist" 
DATA_DIR = os.path.join(PROJECT_PATH, DATA_FOLDER_NAME) 
...
def board_from_input(input_data):
    """
    Takes a 22x8x8 numpy array and converts it back into a python-chess Board object.
    """
    ...
    # Set game state from the remaining planes
    ...
    if np.any(input_data[19, :, :]):
        ep_square_r, ep_square_c = np.unravel_index(np.argmax(input_data[19, :, :]), (8, 8))
        board.ep_square = chess.square(ep_square_c, ep_square_r)

    # Note: Full move, half move, and repetition counters are part of the input
    # but are not needed to reconstruct the visual board (FEN).
...
try:
    ...
    positions_to_view = [0, 25, 35, 45, 55, 1324, 780, 32345, 100, 1000, 5000, 10000] 
    # <<< -------------------------------- >>>
    ...
    for position_index in positions_to_view:
        if 0 <= position_index < len(inputs):
            ...
            # Find and display all moves with non-zero probability
